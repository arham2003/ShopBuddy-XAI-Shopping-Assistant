...
export default function AgentActivityFeed({ steps }) {
  return (
    <div className="max-w-lg mx-auto bg-white rounded-xl shadow-md p-6 space-y-4">
      <h2 className="text-lg font-semibold font-lexend text-slate-900 flex items-center gap-2">
      ...
      <ul className="space-y-3">
        {steps.map((step) => {
          ...
          return (
            <li key={step.key} className={`flex items-center gap-3 ${config.textClass}`}>
              {config.icon}
